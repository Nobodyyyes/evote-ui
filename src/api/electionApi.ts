...
export async function getElectionResults(id: Id): Promise<ElectionResult> {
  if (USE_MOCKS) {
    ...
    return normalizeElectionResult({
      id: election?.id,
      electionId: election?.id,
      electionTitle: election?.name,
      totalVotes: election?.participants,
      totalOptions: election?.options.length,
      resultHash: election?.resultHash,
      calculatedAt: election?.endDateTime,
      optionResults: election?.options.map(option => ({
        optionId: option.id,
