...
export async function getBlockchainRecords(relatedObjectId?: Id): Promise<BlockchainRecord[]> {
    ...
    const payload = await apiFetch<unknown>(
        `/blockchain/records/${encodeURIComponent(relatedObjectId)}`
    )
...
export async function checkIntegrity(
    ...
    if (objectType === 'VOTE') {
        return apiFetch<IntegrityVerificationResponse>(
            `/integrity/votes/${encodeURIComponent(relatedObjectId)}/verify`
        )
    }

    if (objectType === 'RESULT') {
        return apiFetch<IntegrityVerificationResponse>(
            `/integrity/results/${encodeURIComponent(relatedObjectId)}/verify`
        )
    }
