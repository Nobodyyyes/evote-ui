...
function normalizeUserStatus(record: Record<string, unknown>): UserStatus {
    const rawStatus =
        record.status ??
        record.userStatus ??
    ...
    if (rawStatus !== undefined && rawStatus !== null) {
        const status = asString(rawStatus, 'ACTIVE').trim().toUpperCase()
    ...
    if (record.blocked === true || record.isBlocked === true) {
        return 'BLOCKED'
    }

    if (record.active === false || record.enabled === false || record.isActive === false) {
        return 'BLOCKED'
    }
...
export function normalizeUser(payload: unknown): User {
    ...
    return {
        ...
        role,
        status: normalizeUserStatus(record),
